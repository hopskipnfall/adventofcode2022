...
val input = readFile("data/day3.txt")
...
println(
  input
    ...
    .sumOf {
      ...
      if (single.isLowerCase()) -('a'.code - single.code) + 1
      else -('A'.code - single.code) + 1 + 26
    }
)
...
println(
  input
    ...
    .sumOf { group ->
      val single =
        group[0].toSet().intersect(group[1].toSet()).intersect(group[2].toSet()).single()
      if (single.isLowerCase()) -('a'.code - single.code) + 1
      else -('A'.code - single.code) + 1 + 26
    }
)
